  ...
  new TwitterStrategy(
    ...
    async (token, tokenSecret, profile, done) => {
      try {
        ...
        return done(
          new Error("Failed to authenticate with Twitter. Please try again."),
          null
        );
      }
    }
...
export const twitterAuthCallback = (req, res) => {
  passport.authenticate("twitter", (err, data) => {
    if (err || !data) {
      res.redirect(`${process.env.FRONTEND_URL || PORT}/login`);
    ...
    res.redirect(
      `${process.env.FRONTEND_URL || PORT}/login-success?token=${data.token}`
    );
  })(req, res);
};
